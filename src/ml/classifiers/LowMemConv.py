...
class LowMemConvBase(nn.Module):
    ...
    def update_embedding_weights(self, embd_weights: dict, padding_idx=0):
        if padding_idx == 0:
            for i in range(256):
                self.embd.weight.data[i+1] = torch.FloatTensor(embd_weights[str(i)])
        elif padding_idx == 256:
            for i in range(256):
                self.embd.weight.data[i] = torch.FloatTensor(embd_weights[str(i)])
        else:
